...
const VIEWS: ViewDefinition[] = [
  {
    label: 'Scenarios',
    javaTypeSuffix: 'SimulationScenarioJpaEntity',
    statusKeys: ['status'],
    columns: [
      { key: 'code', label: 'Code' },
      { key: 'nameFr', label: 'Name' },
      { key: 'scenarioTypeId', label: 'Scenario type' },
      { key: 'modelId', label: 'Model' },
      { key: 'topologySnapshotId', label: 'Topology snapshot' },
      { key: 'status', label: 'Status' },
    ],
  },
  {
    label: 'Runs',
    javaTypeSuffix: 'SimulationRunJpaEntity',
    statusKeys: ['status'],
    columns: [
      { key: 'scenarioId', label: 'Scenario' },
      { key: 'runTypeId', label: 'Run type' },
      { key: 'status', label: 'Status' },
      { key: 'queuedAt', label: 'Queued at' },
      { key: 'completedAt', label: 'Completed at' },
      { key: 'durationMillis', label: 'Duration (ms)' },
    ],
  },
  {
    label: 'Result summaries',
    javaTypeSuffix: 'SimulationResultSummaryJpaEntity',
    statusKeys: [],
    columns: [
      { key: 'runId', label: 'Run' },
      { key: 'feasible', label: 'Feasible' },
      { key: 'objectiveScore', label: 'Objective score' },
      { key: 'constraintViolationCount', label: 'Constraint violations' },
      { key: 'warningCount', label: 'Warnings' },
      { key: 'resultStatusId', label: 'Result status' },
    ],
  },
  {
    label: 'Result values',
    javaTypeSuffix: 'SimulationResultValueJpaEntity',
    statusKeys: [],
    columns: [
      { key: 'runId', label: 'Run' },
      { key: 'targetType', label: 'Target type' },
      { key: 'targetId', label: 'Target' },
      { key: 'metricCode', label: 'Metric' },
  ...
  {
    label: 'Result series',
    javaTypeSuffix: 'SimulationResultSeriesReferenceJpaEntity',
    statusKeys: [],
    columns: [
      { key: 'runId', label: 'Run' },
...
function SimulationResourceView({
  definition,
  resource,
  canRead,
}: {
  ...
  return (
    <Stack spacing={2}>
      <Typography color="text.secondary" variant="body2">
        Runtime resource: {resource.resource}. Simulation records and results are decision-support evidence; they do not change operational source-of-truth modules.
      </Typography>
      ...
      {selectedId ? (
        <Paper variant="outlined" sx={{ p: 2 }}>
          <Stack spacing={1}>
            <Typography component="h3" variant="h6">Simulation detail</Typography>
            {detailQuery.isPending ? <CircularProgress size={20} /> : null}
            {detailQuery.isError ? <Alert severity="error">{errorMessage(detailQuery.error, 'simulation detail')}</Alert> : null}
            {detailQuery.data ? (
...
export function SimulationWorkspacePage() {
  ...
  if (permissions.status !== 'ready') {
    return <Container maxWidth="xl"><Alert severity="info">Loading simulation permissions…</Alert></Container>;
  }

  if (!listPermission || !detailPermission) {
    return (
      <Container maxWidth="xl">
        <Alert severity="warning">Simulation workbench route-permission metadata is unavailable. Access is denied by default.</Alert>
      </Container>
    );
  }
  ...
  if (!canRead) {
    return (
      <Container maxWidth="xl">
        <Alert severity="warning">Your current HidraAPI grants do not allow simulation workbench reads.</Alert>
      </Container>
    );
  }
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 3 }}>
      <Stack spacing={3}>
        <Box>
          <Typography component="h1" variant="h4">Simulation intelligence</Typography>
          <Typography color="text.secondary">
            HWEB-013 simulation scenarios, runs, and results backed by runtime workbench discovery. Simulation outputs remain decision-support evidence rather than operational commands.
          </Typography>
        </Box>
        ...
        <Paper variant="outlined">
          <Tabs aria-label="Simulation views" value={tab} onChange={(_, value: number) => setTab(value)} variant="scrollable" scrollButtons="auto">
            {VIEWS.map((view) => <Tab key={view.label} label={view.label} />)}
          </Tabs>
        </Paper>

        <SimulationResourceView definition={VIEWS[tab]} resource={resourcesByView[tab]} canRead={canRead} />
      </Stack>
    </Container>
  );
}
