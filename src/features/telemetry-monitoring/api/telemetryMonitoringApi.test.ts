...
describe('HWEB-006 telemetry monitoring API boundary', () => {
  ...
  it('uses only the published telemetry and monitoring routes', async () => {
    ...
    await fetchDeviations({ severity: 'HIGH', page: 0, size: 50 });
    ...
    expect(client).toHaveBeenNthCalledWith(7, { method: 'GET', url: '/api/v1/monitoring/deviations', params: { severity: 'HIGH', page: 0, size: 50 } });
  });
});
