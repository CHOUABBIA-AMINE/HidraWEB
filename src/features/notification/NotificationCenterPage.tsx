...
export function NotificationCenterPage() {
  ...
  const deliveryResource = useMemo(() => resources.find((r) => r.javaType.endsWith('NotificationDeliveryAttemptJpaEntity')), [resources]);
  ...
  const deliveryQuery = useQuery({ queryKey: workbenchQueryKeys.list(MODULE, deliveryResource?.resource ?? '', 0, PAGE_SIZE, ''), queryFn: () => fetchWorkbenchRecords({ module: MODULE, resource: deliveryResource?.resource ?? '', page: 0, size: PAGE_SIZE, query: '' }), enabled: canRead && Boolean(deliveryResource) });
  ...
  const missing = [messageResource, requestResource, deliveryResource].filter((descriptor) => !descriptor).length;
  const firstError = [messageQuery, requestQuery, deliveryQuery].find((query) => query.isError)?.error;

  return (
    <Container maxWidth="xl" sx={{ py: 3 }}><Stack spacing={3}>
      <Box>
        <Typography component="h1" variant="h4">Notification center</Typography>
        <Typography color="text.secondary">HWEB-014-05 notification message, request, and delivery-attempt evidence from runtime workbench resources.</Typography>
      </Box>
      <Alert severity="info">This center is evidence-only. HidraAPI publishes no inbox read/unread, acknowledge, dismiss, archive, delete, resend, retry, preference, or real-time notification endpoint, so none of those behaviors is exposed here.</Alert>
      ...
      {deliveryResource ? <EvidenceTable title={`Delivery attempts — ${deliveryResource.resource}`} items={deliveryQuery.data?.items ?? []} /> : null}
    </Stack></Container>
  );
}
