...
test('HWEB-014-03 reads runtime document evidence and submits published metadata contracts only', async ({ page }) => {
  ...
  await page.getByLabel('code').fill('DOC-002');
  await page.getByLabel('titleFr').first().fill('Procédure');
  await page.getByLabel('confidentialityLevel').fill('2');
  await page.getByLabel('ownerModule').fill('integrity');
  await page.getByLabel('createdByActorId').fill('actor-1');
  ...
  await page.getByLabel('documentId').first().fill('doc-2');
  await page.getByLabel('versionLabel').fill('v1');
  await page.getByLabel('storageObjectId').fill('object-77');
  await page.getByLabel('mimeType').fill('application/pdf');
  await page.getByLabel('originalFilename').fill('procedure.pdf');
  await page.getByLabel('fileSizeBytes').fill('1024');
  await page.getByLabel('checksumAlgorithm').fill('SHA-256');
  await page.getByLabel('checksumValue').fill('abc123');
  await page.getByLabel('uploadedByActorId').fill('actor-1');
  ...
  await page.getByLabel('documentId').nth(1).fill('doc-2');
  await page.getByLabel('documentVersionId').fill('version-1');
  await page.getByLabel('targetModule').fill('integrity');
  await page.getByLabel('targetTypeCode').fill('PIPELINE');
  await page.getByLabel('targetId').fill('pipe-1');
  await page.getByLabel('primaryLink').check();
  await page.getByLabel('linkedByActorId').fill('actor-1');
